...
namespace Roadkill.Core.Services
{
    ...
    public class SearchService : ServiceBase
    {
        ...
        public virtual void Add(PageViewModel model)
        {
            try
            {
                ...
                using (IndexWriter writer = new IndexWriter(FSDirectory.Open(new DirectoryInfo(IndexPath)), analyzer, false, IndexWriter.MaxFieldLength.UNLIMITED))
                {
                    ...
                    document.Add(new Field("projectlanguage", model.ProjectLanguage?? string.Empty, Field.Store.YES, Field.Index.ANALYZED));
            ...
            try
            {
                ...
                using (IndexWriter writer = new IndexWriter(FSDirectory.Open(new DirectoryInfo(IndexPath)), analyzer, true, IndexWriter.MaxFieldLength.UNLIMITED))
                {
                    foreach (Page page in Repository.AllPages().ToList())
                    {
                        ...
                        document.Add(new Field("projectlanguage", pageModel.ProjectLanguage?? string.Empty, Field.Store.YES, Field.Index.ANALYZED));
                        document.Add(new Field("projectstatus", pageModel.ProjectStatus ?? string.Empty, Field.Store.YES, Field.Index.ANALYZED));
                        document.Add(new Field("Projectagilelifecyclephase", pageModel.ProjectAgileLifeCyclePhase ?? string.Empty, Field.Store.YES, Field.Index.ANALYZED));
        ...
        //public IList<Tuple<string, int>> GetTagCloud(IList<string> modelTags)
        //{
        ...
        //    if (modelTags.Count == 0) return list;

        //    StandardAnalyzer analyzer = new StandardAnalyzer(LUCENEVERSION);
        //    MultiFieldQueryParser parser = new MultiFieldQueryParser(LuceneVersion.LUCENE_29, new string[] { "tags" }, analyzer);
        ...
        //        catch (Lucene.Net.QueryParsers.ParseException)
        //        {
        //            query = parser.Parse(QueryParser.Escape(tag));
        //        }
        ...
        //                    if (count > 5) count = 5;
        //                    list.Add(new Tuple<string, int>(tag, count));
        //                }
        //            }
        //            catch (FileNotFoundException)
        //            {
